...
// GET — list batches with row counts
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  const map: Record<string, {
    batch_id: string; batch_name: string; source: string;
    sender_profile_id: string | null; sender_profile_name: string | null;
    uploaded_at: string; count: number;
  }> = {};
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  const senderProfileId = (formData.get("sender_profile_id") as string | null) || null;
